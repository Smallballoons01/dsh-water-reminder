/**
 * The embedded skill shipped by @dsh-plugin/water-reminder.
 *
 * The plugin registers this skill at runtime through `ctx.skills.register()`,
 * so the model gets hydration coaching instructions the moment the bundle is
 * mounted. The same text is also written to
 * `skills/hydration-coach/SKILL.md` for deployments that prefer a filesystem
 * skill root (`~/.dsh/skills`, `~/.agents/skills`, a project `.dsh/skills`).
 *
 * This module is the single source of truth: `test/skill.test.js` asserts that
 * the checked-in SKILL.md is byte-identical to {@link skillMarkdown}, so the
 * two delivery paths cannot drift apart.
 *
 * @module water-reminder/skill
 */

/** Kebab-case skill name, as the skill registry requires. */
export const SKILL_NAME = 'hydration-coach'

/** Routing description shown in the model-facing skill catalog. */
export const SKILL_DESCRIPTION = 'Hydration coaching over the water-reminder plugin tools: log drinks, tune the 40-60 minute reminder rhythm, and answer "how much water today".'

/** Extra routing guidance for the catalog. */
export const SKILL_WHEN_TO_USE = 'Use when the user mentions water, drinking, hydration, cups, 喝水, or when a water-reminder nudge fires and you need to react to it.'

/**
 * The skill body, in Markdown, without frontmatter.
 * @returns the instruction body.
 */
export function skillBody() {
  return `# Hydration coach
...
/**
 * The complete SKILL.md file contents: YAML frontmatter plus the body.
 * @returns the Markdown document, newline-terminated.
 */
export function skillMarkdown() {
  ...
  // The blank line between the closing fence and the body is what every other
  // skill file in the ecosystem uses; frontmatter parsers rely on it.
  return `${frontmatter}\n\n${skillBody()}`
}
